...
## Troubleshooting: Cached Versions

**Problem:** After publishing a new claude-team version to PyPI, the launchd service may continue running an old cached version.

**Symptoms:**
- New features/fixes not working despite confirmed PyPI release
- `uv tool install claude-team-mcp` shows old version
...
pkill -9 -f "claude_team\|uvx.*claude-team"

# 3. Force clean uv cache
uv cache clean --force

# 4. Reinstall with refresh
uv tool install --force --refresh claude-team-mcp

# 5. Verify correct version
uv tool list | grep claude

# 6. Restart service
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/com.claude-team.plist
```

**Why this happens:** `uvx` caches tool environments aggressively. Even after a PyPI publish, the local cache may serve the old version until explicitly cleared.

## Important: Start Worktrees from Main

When spawning workers for new beads tasks, ensure the worktree is created from `main` (or the appropriate base branch), not from whatever branch the main repo happens to be on.

**Problem:** If the main repo is checked out to a feature branch, worktrees inherit that as their base. This causes PRs to include unrelated commits.

**Solution:** Workers should either:
1. Explicitly checkout main before creating worktree
2. Or specify the base branch when creating the worktree

**Lesson learned:** Cherry-picking to a clean branch works but adds friction. Better to start clean.
