...
describe("trigger handling", () => {
  ...
  it("orders providers in /models list", async () => {
    await withTempHome(async (home) => {
      const cfg = makeCfg(home);
      const res = await getReplyFromConfig(
        {
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      const normalized = normalizeTestText(text ?? "");
      const anthropicIndex = normalized.indexOf("- anthropic ");
      const openrouterIndex = normalized.indexOf("- openrouter ");
      const openaiIndex = normalized.indexOf("- openai ");
      const codexIndex = normalized.indexOf("- openai-codex ");
      const minimaxIndex = normalized.indexOf("- minimax ");
      ...
      expect(anthropicIndex).toBeLessThan(minimaxIndex);
      expect(minimaxIndex).toBeLessThan(openaiIndex);
      expect(openaiIndex).toBeLessThan(codexIndex);
      expect(codexIndex).toBeLessThan(openrouterIndex);
    });
  });
